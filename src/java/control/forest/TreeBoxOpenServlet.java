...
@WebServlet(name = "TreeBoxOpenServlet", urlPatterns = {"/openTreeBox"})
public class TreeBoxOpenServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        UserInventoryItemService userInventoryItemService = new UserInventoryItemService(em);
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            // Get all user inventory items for this user
            System.out.println("Fetching inventory items for user: " + userId);
            List<Userinventoryitem> userItems = userInventoryItemService.findByUserId(userId);
            System.out.println("Found " + userItems.size() + " inventory items for user");

            // Check if the user already has this tree
            boolean treeExists = false;
            String treeInventoryItemId = null;
            String treeboxInventoryItemId = null;

            System.out.println("NOW I WILL FIND THE TREEBOX\n\n");

            for (Userinventoryitem item : userItems) {
                // Find the treebox inventory item
                if (item.getTreeboxid() != null) {
                    System.out.println("Is it null?" + item.getTreeboxid());
                    System.out.println("item.getTreeboxid().getTreeboxid() is " + item.getTreeboxid().getTreeboxid());
                    System.out.println("User selected treeboxid is " + treeboxId);
                    System.out.println("item.getIsdeteled() is " + item.getIsdeleted());

                    if (item.getTreeboxid().getTreeboxid().equals(treeboxId)
                            && item.getIsdeleted() == false) {
                        System.out.println("I found: " + item.getInventoryitemid() + " " + item.getUserid() + " " + item.getTreeboxid());
                        treeboxInventoryItemId = item.getInventoryitemid();
                        System.out.println("Found treebox inventory item ID: " + treeboxInventoryItemId);
                        break;
                    }
                } else {
                    continue;
            ...
            System.out.println("Checking if user already has this tree and finding treebox item");
            for (Userinventoryitem item : userItems) {
                if (item.getTreeid() != null) {
                    System.out.println(item.getInventoryitemid() + " " + item.getUserid() + " " + item.getTreeboxid());
                    // Check for existing tree
                    if (item.getTreeid().getTreeid().equals(selectedTree.getTreeid())
                            && item.getIsdeleted() == false) {
                        System.out.println("I found tree: " + item.getInventoryitemid() + " " + item.getUserid() + " " + item.getTreeboxid());
                        treeInventoryItemId = item.getInventoryitemid();
                        treeExists = true;
                        System.out.println("User already has this tree, inventory item ID: " + treeInventoryItemId);
                        break;
                    }
                }
            }
            ...
            try {
                ...
                // Update or create tree inventory item
                if (treeExists) {
                    System.out.println("Updating existing tree inventory item");
                    // Increase quantity of existing tree
                    Userinventoryitem existingItem = userInventoryItemService.findById(treeInventoryItemId);
                    ...
                    System.out.println("Tree inventory item updated successfully");
                } else {
                    System.out.println("Creating new tree inventory item");
                    // Create new inventory item for the tree
                    Userinventoryitem newItem = new Userinventoryitem();
                    // Generate a sequential ID using the IVAA0001 pattern
                    String newItemId = generateNextInventoryItemId(userInventoryItemService);
                    System.out.println("Generated new inventory item ID: " + newItemId);
                    System.out.println("the record eh treeid " + selectedTree);

                    newItem.setInventoryitemid(newItemId);
                    newItem.setTreeid(selectedTree);
                    newItem.setTreeboxid(null);
                    newItem.setItemid(null);
                    newItem.setBiomeid(null);
                    newItem.setQuantity(1);
                    newItem.setIsdeleted(false);
                    newItem.setUserid(userService.findUserById(userId));
                    System.out.println("new item id " + newItem.getInventoryitemid());
                    System.out.println("new item's tree id " + newItem.getTreeid());
                    System.out.println("new item quantity " + newItem.getQuantity());
                    System.out.println("new is deleted " + newItem.getIsdeleted());

                    System.out.println("NEW ITEM: " + newItem);

                    userInventoryItemService.addUserInventoryItem(newItem);
                    System.out.println("New tree inventory item created successfully");
                }

                // Decrease quantity of tree box
                if (treeboxInventoryItemId != null) {
                    System.out.println("Updating treebox inventory item");
                    Userinventoryitem treeboxItem = userInventoryItemService.findById(treeboxInventoryItemId);
                    System.out.println("treeboxitem record " + treeboxItem);
                    System.out.println("Current treebox quantity: " + treeboxItem.getQuantity());

                    if (treeboxItem.getQuantity() > 1) {
                        treeboxItem.setQuantity(treeboxItem.getQuantity() - 1);
                        userInventoryItemService.updateUserInventoryItem(treeboxItem);
                        System.out.println("Treebox inventory item updated successfully " + treeboxItem);
                        System.out.println("New treebox quantity: " + treeboxItem.getQuantity());
                    } else {
                        // If quantity becomes 0, mark as deleted or remove
                        System.out.println("Treebox quantity will be 0, marking as deleted");
                        userInventoryItemService.deleteUserInventoryItem(treeboxInventoryItemId);
                        System.out.println("Treebox inventory item marked as deleted");
                    }
                } else {
                    // Handle error: user doesn't have this tree box
                    System.out.println("ERROR: User doesn't have this tree box in inventory");
                    utx.rollback(); // Roll back the transaction
                    sendErrorResponse(out, "User doesn't have this tree box in inventory");
                    return;
                }
                ...
                // Send error response
                sendErrorResponse(out, "Transaction error: " + e.getMessage());
                return;
            }
