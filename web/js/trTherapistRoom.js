document.addEventListener("DOMContentLoaded", function(){
    ...
    this.querySelectorAll(".chat-item").forEach(item => {
        item.addEventListener("click", function (e) {
            const newUserId = e.target.getAttribute('data-userid');
            ...
            const newChatBox = document.querySelector(`.chat-wrapper[data-userid="${newUserId}"]`);
            if (newChatBox) {
    ...
    document.querySelectorAll(".chat-wrapper").forEach(chatBox => {
        const sendBtn = chatBox.querySelector(".send-btn");
        const textarea = chatBox.querySelector(".input-message textarea");
    ...
    function sendMessage(oriMessage) {
        const receiverUserId = document.querySelector(".active-chat").getAttribute("data-userid");
        
        const firstPath = location.pathname.split('/')[1];
        const message = oriMessage.trim();
        if (message === ""){
            ...
            $.ajax({
                ...
                success: function (response) {
                    if (response) {
                        const chatBox = document.querySelector(".chat-wrapper.chat-box-active .chat-box");
